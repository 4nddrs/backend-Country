...
async def create_total_control(control_in: schemas_control.TotalControlCreate):
    ...
    if not control:
        raise HTTPException(
            ...
            detail="No se pudo crear el registro de control total."
        )
    return control
...
@router.get(
    "/", 
    response_model=List[schemas_control.TotalControl],
    status_code=status.HTTP_200_OK
)
async def list_total_controls(skip: int = 0, limit: int = 100):
    controls = await crud_control.get_total_controls(skip=skip, limit=limit)
    return controls
...
async def get_total_control(idTotalControl: int):
    ...
    if not control:
        raise HTTPException(
            ...
            detail="Registro de control total no encontrado."
        )
    return control
...
async def update_total_control(
    idTotalControl: int,
    control_in: schemas_control.TotalControlUpdate
):
    updated_control = await crud_control.update_total_control(idTotalControl, control_in)
    if not updated_control:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No se encontró el registro con ID {idTotalControl} para actualizar."
        )
...
async def delete_total_control(idTotalControl: int):
    ...
    if not deleted_control:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No se encontró el registro con ID {idTotalControl} para eliminar."
        )
